...
MANDATORY SEQUENCE - DO NOT SKIP ANY STEP OR STOP EARLY:
...
7. **CRITICAL**: ALWAYS proceed to RiskAssessment with format: user_data_json|loan_amount (NEVER stop after GeoPolicyCheck)
8. Final decision based on BOTH GeoPolicyCheck AND RiskAssessment results

IMPORTANT: Even if GeoPolicyCheck returns conditions or warnings, you MUST still run RiskAssessment. 
The final loan decision should be based on the combination of both assessments, not just one.
DO NOT make early loan rejections based solely on GeoPolicyCheck conditions.
