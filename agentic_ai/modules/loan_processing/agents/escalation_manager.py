...
class EscalationManager:
    ...
    def execute_with_escalation(self,
        ...
        Args:
            ...
            step_id: Optional logical step identifier for stable attempt tracking
       
        Returns:
            Agent response or human escalation response
        """
        # Use step_id for stable session_key if provided
        if step_id:
            session_key = f"{agent_name}_{step_id}"
        else:
    ...
    def process_human_response(self, escalation_id: str, response: str) -> bool:
        """
        Process and log a human operator's response to an escalation.
       
        Args:
            escalation_id: The unique ID of the escalation/case
            response: The human operator's response
           
        Returns:
            True if the response was logged successfully, False otherwise
        """
        try:
            from agentic_ai.modules.loan_processing.agents.human_agent import get_human_agent
            human_agent = get_human_agent()
            responses = human_agent._load_human_responses()
            # Format timestamp as 'Jul 01, 2025, 05:41 PM'
            formatted_time = datetime.now().strftime('%b %d, %Y, %I:%M %p')
            responses[escalation_id] = {
                "response": response,
                "timestamp": formatted_time
            }
            human_agent._save_human_responses(responses)
           
            # Update escalation status to resolved
            active_sessions = human_agent._load_active_sessions()
            if escalation_id in active_sessions:
                active_sessions[escalation_id]['status'] = 'resolved'
                active_sessions[escalation_id]['human_response'] = {
                    "response": response,
                    "timestamp": formatted_time
                }
                active_sessions[escalation_id]['resolved_at'] = datetime.now().isoformat()
                human_agent._save_active_sessions(active_sessions)
           
            return True
        except Exception as e:
            print(f"Error logging human response: {e}")
            return False
